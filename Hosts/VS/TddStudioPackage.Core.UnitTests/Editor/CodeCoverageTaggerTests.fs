module R4nd0mApps.TddStud10.Hosts.VS.TddStudioPackage.Core.Editor.CodeCoverageTaggerTests
...
let createCCT2 (tb : ITextBuffer) (DocumentCoordinate ln) sps = 
    ...
    let tss = sps |> List.map (fun sp -> TagSpan<SequencePointTag>(nssc.[0], { SptSequencePoint = sp }))
